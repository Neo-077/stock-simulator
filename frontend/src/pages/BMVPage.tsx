...
const BMVPage: React.FC = () => {
  ...
  const list = marketData.bmv.filter(s => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Bolsa Mexicana de Valores (BMV)</h1>
        <p className="text-gray-600">10 acciones principales del mercado mexicano</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-4 mb-6">
        ...
        <div className="mt-4 flex items-center gap-6 text-sm text-gray-600">
          <span className="font-medium text-gray-900">
            Total: <span className="text-blue-600">{marketData.bmv.length}</span>
          </span>
          <span className="font-medium text-gray-900">
            En alza: <span className="text-green-600">{marketData.bmv.filter(s => s.changePct > 0).length}</span>
          </span>
          <span className="font-medium text-gray-900">
            En baja: <span className="text-red-600">{marketData.bmv.filter(s => s.changePct < 0).length}</span>
          </span>
          <span className="font-medium text-gray-900">
            Mostrando: <span className="text-blue-600">{list.length}</span>
          </span>
        </div>
      </div>
